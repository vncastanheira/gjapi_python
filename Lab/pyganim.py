...
class PygAnimation(object):
    ...
    def blitFrameNumStatic(self, frameNum, destSurface, dest):
        # Draws the specified frame of the animation object. This (REALLY) ignores the
        # current playing state.
        #
        # @param frameNum
        #     The frame to draw (the first frame is 0, not 1)
    ...
    def getRect(self, inX=0, inY=0):
        # Returns a pygame.Rect object for this animation object.
        # The top and left will be set to (0, 0) if argument is passed
        # The width and height will be set to what is returned by getMaxSize().
        maxWidth, maxHeight = self.getMaxSize()
        return pygame.Rect(inX, inY, maxWidth, maxHeight)


    def getRectRotated(self, inX=0, inY=0):
        # Returns a pygame.Rect object for an rotated animation object.
        # Gets the transformed image and returns the corresponding Rect object.
        return self._transformedImages[0].get_rect()
